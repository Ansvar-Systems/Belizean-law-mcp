#!/usr/bin/env tsx
/**
 * Belize Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from agm.gov.bz (PDF downloads).
 *
 * Pipeline per law:
 *   1. Download PDF from agm.gov.bz/uploads/laws/...
 ...
 *
 * Data source: agm.gov.bz (Attorney General's Ministry)
...
import { parseBZLawPdf, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Belize Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('=====================================================\n');
  ...
  for (const law of acts) {
    ...
    try {
      ...
      // Parse PDF
      const parsed = parseBZLawPdf(sourceFile, act);
  ...
  console.log(`\n  Source:      agm.gov.bz (PDF extraction)`);
