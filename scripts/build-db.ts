#!/usr/bin/env tsx
/**
 * Database builder for Belize Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Belize Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Belizean statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'BZ');
    insertMeta.run('source', 'agm.gov.bz');
